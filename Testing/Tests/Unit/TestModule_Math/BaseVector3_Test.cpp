...
ZTEST_SUITE_BEGIN( BaseVector3_TestSuite )
...
ZTEST_CASE ( Constructor2_ValuesAreCopiedProperly_Test )
{
    ...
    const BaseVector3 VECTOR_TO_COPY(EXPECTED_VALUE_FOR_X, EXPECTED_VALUE_FOR_Y, EXPECTED_VALUE_FOR_Z);

	// [Execution]
    BaseVector3 vVectorUT = VECTOR_TO_COPY;
...
ZTEST_CASE ( Constructor3_ValuesAreSetProperly_Test )
{
	...
	// [Execution]
    BaseVector3 vVectorUT(EXPECTED_VALUE_FOR_X, EXPECTED_VALUE_FOR_Y, EXPECTED_VALUE_FOR_Z);
...
ZTEST_CASE ( Constructor4_ValueIsSetForAllComponents_Test )
{
	...
	// [Execution]
    BaseVector3 vVectorUT(EXPECTED_VALUE_FOR_ALL);
...
ZTEST_CASE ( Constructor5_VectorComponentsAreFilledWithValidReferenceTo3Floats_Test )
{
	...
	// [Execution]
    BaseVector3 vVectorUT(VALID_ARRAY_OF_3_FLOATS);
...
ZTEST_CASE ( Constructor5_AssertionFailsWhenPointerIsNull_Test )
{
    ...
    try
    {
        BaseVector3 vVectorUT(NULL_ARRAY);
    }
...
{
    // [Preparation]
    const BaseVector3 LEFT_OPERAND(SFloat::Epsilon);
    const BaseVector3 RIGHT_OPERAND(SFloat::_0);
...
{
    // [Preparation]
    const BaseVector3 LEFT_OPERAND(SFloat::Epsilon - SFloat::Epsilon * SFloat::_0_5);
    const BaseVector3 RIGHT_OPERAND(SFloat::_0);
...
ZTEST_CASE ( OperatorEquality_TrueWhenOperandsAreExactlyEqual_Test )
{
    // [Preparation]
    const BaseVector3 LEFT_OPERAND(SFloat::Epsilon);
    const BaseVector3 RIGHT_OPERAND(SFloat::Epsilon);
