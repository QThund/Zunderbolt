...
// !!!
// IMPORTANT NOTE: Whatever the test you modify in this file, replicate the same changes in the Ray_Vector4_Test.cpp file!
...
ZTEST_CASE_TEMPLATE ( Constructor2_ValuesAreCopiedProperly_Test, TTemplateTypes )
{
    ...
    Ray<T, T> EXPECTED_RAY(EXPECTED_VALUE_FOR_ORIGIN, EXPECTED_VALUE_FOR_DIRECTION);

	// [Execution]
    Ray<T, T> rayUT(EXPECTED_RAY);
...
ZTEST_CASE_TEMPLATE ( Constructor4_ValuesAreSetProperly_Test, TTemplateTypes )
{
	...
	// [Execution]
    Ray<T, T> rayUT(EXPECTED_VALUE_FOR_ORIGIN, EXPECTED_VALUE_FOR_DIRECTION);
...
ZTEST_CASE_TEMPLATE ( Constructor4_ConstructedRayIsNotNormalizedWhenDirectionVectorIsNot_Test, TTemplateTypes )
{
	...
	// [Execution]
    Ray<T, T> rayUT(INPUT_VALUE_FOR_ORIGIN, INPUT_VALUE_FOR_DIRECTION);
...
{
    // [Preparation]
    Ray<T, T> EXPECTED_VALUE(T::GetNullVector(), T::GetNullVector());

	// [Execution]
    Ray<T, T> obtinedRay = Ray<T, T>::GetNullRay();
...
{
    // [Preparation]
    const Ray<T, T> LEFT_OPERAND = Ray<T, T>( T(SFloat::Epsilon), T(SFloat::Epsilon) );
    const Ray<T, T> RIGHT_OPERAND = Ray<T, T>( T(SFloat::_0), T(SFloat::_0) );
...
{
    // [Preparation]
    const Ray<T, T> LEFT_OPERAND = Ray<T, T>( T(SFloat::Epsilon - SFloat::Epsilon * SFloat::_0_5), T(SFloat::Epsilon - SFloat::Epsilon * SFloat::_0_5));
    const Ray<T, T> RIGHT_OPERAND = Ray<T, T>( T(SFloat::_0), T(SFloat::_0) );
...
ZTEST_CASE_TEMPLATE ( OperatorEquality_TrueWhenOperandsAreExactlyEqual_Test, TTemplateTypes )
{
    // [Preparation]
    const Ray<T, T> LEFT_OPERAND = Ray<T, T>( T(SFloat::Epsilon), T(SFloat::Epsilon) );
    const Ray<T, T> RIGHT_OPERAND = Ray<T, T>( T(SFloat::Epsilon), T(SFloat::Epsilon) );
...
ZTEST_CASE_TEMPLATE ( OperatorEquality_FalseWhenOperandsAreSimilarButNotEqual_Test, TTemplateTypes )
{
    // [Preparation]
    const Ray<T, T> LEFT_OPERAND = Ray<T, T>( T(SFloat::Epsilon), T(SFloat::Epsilon) );
    const Ray<T, T> RIGHT_OPERAND = Ray<T, T>( T(SFloat::Epsilon), T(SFloat::Epsilon) * SFloat::_3 ); // LEFT_OPERAND's direction x3
...
ZTEST_CASE_TEMPLATE ( OperatorAssignation_RayIsAssignedProperlyToAnother_Test, TTemplateTypes )
{
    // [Preparation]
    float_z VECTOR_COMPONENTS_ORIGIN[] = { SFloat::_1, SFloat::_2, SFloat::_3 };
    float_z VECTOR_COMPONENTS_DIRECTION[] = { SFloat::_1, SFloat::_2, SFloat::_3 };
    const T EXPECTED_VALUE_FOR_ORIGIN(VECTOR_COMPONENTS_ORIGIN);
    const T EXPECTED_VALUE_FOR_DIRECTION(VECTOR_COMPONENTS_DIRECTION);

    const Ray<T, T> OTHER_RAY = Ray<T, T>(EXPECTED_VALUE_FOR_ORIGIN, EXPECTED_VALUE_FOR_DIRECTION);
...
ZTEST_CASE_TEMPLATE ( Invert_CorrectValueIsReturnedForACommonRay_Test, TTemplateTypes )
{
    ...
    const Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
    const Ray<T, T> EXPECTED_RAY = Ray<T, T>(RAY.Origin, -RAY.Direction);

	// [Execution]
    Ray<T, T> invertedRay = RAY.Invert();
...
ZTEST_CASE_TEMPLATE ( Normalize_CorrectValueIsReturnedForACommonRay_Test, TTemplateTypes )
{
    ...
    const Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
    const Ray<T, T> EXPECTED_RAY = Ray<T, T>(RAY.Origin, RAY.Direction.Normalize());

	// [Execution]
    Ray<T, T> normalizedRay = RAY.Normalize();
...
ZTEST_CASE_TEMPLATE ( GetPoint_ExpectedPointIsReturnedForACommonDistance_Test, TTemplateTypes )
{
    ...
    Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
    RAY = RAY.Normalize();
...
ZTEST_CASE_TEMPLATE ( GetPoint_AssertionFailsWhenRayIsNotNormalized_Test, TTemplateTypes )
{
    ...
    const Ray<T, T> NOT_NORMALIZED_RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
...
ZTEST_CASE_TEMPLATE ( GetPoint_UnexpectedPointIsReturnedWhenRayIsNotNormalized_Test, TTemplateTypes )
{
    ...
    const Ray<T, T> NOT_NORMALIZED_RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
    const Ray<T, T> NORMALIZED_RAY = NOT_NORMALIZED_RAY.Normalize();
...
ZTEST_CASE_TEMPLATE ( Intersection_ReturnsFalseWhenRayDoesNotIntersectTheOrb_Test, TTemplateTypes )
{
    ...
    float_z VECTOR_COMPONENTS_CENTER[] = { SFloat::_1, SFloat::_2, SFloat::_3 };
...
ZTEST_CASE_TEMPLATE ( Intersection_ReturnsTrueWhenRayIntersectsTheOrbTwice_Test, TTemplateTypes )
{
    ...
    float_z VECTOR_COMPONENTS_CENTER[] = { SFloat::_2, SFloat::_2, SFloat::_3 };
    const T VALUE_FOR_CENTER(VECTOR_COMPONENTS_CENTER);
    const float_z RADIUS = SFloat::_1;
    const Orb<T> ORB = Orb<T>(VALUE_FOR_CENTER, RADIUS);
...
ZTEST_CASE_TEMPLATE ( Intersection_ReturnsTrueWhenOriginLiesOnTheSurfaceOfTheOrb_Test, TTemplateTypes )
{
    ...
    // [Preparation]
    float_z VECTOR_COMPONENTS_ORIGIN[] = { SFloat::_3, SFloat::_2, SFloat::_3 };
    float_z VECTOR_COMPONENTS_DIRECTION[] = { SFloat::_1, SFloat::_0, SFloat::_0 };
    const T VALUE_FOR_ORIGIN(VECTOR_COMPONENTS_ORIGIN);
    const T VALUE_FOR_DIRECTION(VECTOR_COMPONENTS_DIRECTION);
    Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
...
ZTEST_CASE_TEMPLATE ( Intersection_ReturnsTrueWhenOriginIsContainedInTheOrb_Test, TTemplateTypes )
{
    ...
    // [Preparation]
    float_z VECTOR_COMPONENTS_ORIGIN[] = { SFloat::_2, SFloat::_2, SFloat::_3 };
    float_z VECTOR_COMPONENTS_DIRECTION[] = { SFloat::_1, SFloat::_0, SFloat::_0 };
    const T VALUE_FOR_ORIGIN(VECTOR_COMPONENTS_ORIGIN);
    const T VALUE_FOR_DIRECTION(VECTOR_COMPONENTS_DIRECTION);
    Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
...
ZTEST_CASE_TEMPLATE ( Intersection_ReturnsTrueWhenRayIsTangentToTheOrb_Test, TTemplateTypes )
{
    ...
    // [Preparation]
    float_z VECTOR_COMPONENTS_ORIGIN[] = { SFloat::_1, SFloat::_3, SFloat::_3 };
    float_z VECTOR_COMPONENTS_DIRECTION[] = { SFloat::_1, SFloat::_0, SFloat::_0 };
    const T VALUE_FOR_ORIGIN(VECTOR_COMPONENTS_ORIGIN);
    const T VALUE_FOR_DIRECTION(VECTOR_COMPONENTS_DIRECTION);
    Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
...
ZTEST_CASE_TEMPLATE ( Intersection_ReturnsExpectedResultWhenRadiusOfTheOrbEqualsZero_Test, TTemplateTypes )
{
    ...
    // [Preparation]
    const float_z VECTOR_COMPONENTS_ORIGIN[] = { SFloat::_1, SFloat::_2, SFloat::_3 };
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint1_NoIntersectionPointsAreReturnedWhenRayDoesNotIntersectTheOrb_Test, TTemplateTypes )
{
    ...
    const T VALUE_FOR_ORIGIN(VECTOR_COMPONENTS_ORIGIN);
    const T VALUE_FOR_DIRECTION(VECTOR_COMPONENTS_DIRECTION);
    const Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION.Normalize());

    float_z VECTOR_COMPONENTS_CENTER[] = { SFloat::_10, SFloat::_10, SFloat::_10 };
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint1_ReturnsTwoIntersectionPointsWhenRayIntersectsTheOrbTwice_Test, TTemplateTypes )
{
    ...
    float_z VECTOR_COMPONENTS_OUTPUT_POINT[] = { SFloat::_1, SFloat::_2, SFloat::_2 };
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint1_ReturnsOnePointWhenOriginLiesOnTheSurfaceOfTheOrb_Test, TTemplateTypes )
{
    ...
    float_z VECTOR_COMPONENTS_OUTPUT_POINT[] = { SFloat::_3, SFloat::_2, SFloat::_3 };
    const T EXPECTED_POINT = T(VECTOR_COMPONENTS_OUTPUT_POINT);
    T OUTPUT_POINT = T::GetNullVector();
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint1_ReturnsOnePointWhenRayIsTangentToTheOrb_Test, TTemplateTypes )
{
    ...
    // [Preparation]
    float_z VECTOR_COMPONENTS_ORIGIN[] = { SFloat::_0, SFloat::_3, SFloat::_3 };
    float_z VECTOR_COMPONENTS_DIRECTION[] = { SFloat::_1, SFloat::_0, SFloat::_0 };
    const T VALUE_FOR_ORIGIN(VECTOR_COMPONENTS_ORIGIN);
    const T VALUE_FOR_DIRECTION(VECTOR_COMPONENTS_DIRECTION);
    Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
    ...
    const EIntersections EXPECTED_RESULT = EIntersections::E_One;
    float_z VECTOR_COMPONENTS_OUTPUT_POINT[] = { SFloat::_2, SFloat::_3, SFloat::_3 };
    const T EXPECTED_POINT = T(VECTOR_COMPONENTS_OUTPUT_POINT);
    T OUTPUT_POINT = T::GetNullVector();
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint1_ReturnsNoPointsWhenRayDoesNotIntersectTheOrbButWouldDoIfInverted_Test, TTemplateTypes )
{
    ...
    // [Preparation]
    float_z VECTOR_COMPONENTS_ORIGIN[] = { SFloat::_0, SFloat::_2, SFloat::_3 };
    float_z VECTOR_COMPONENTS_DIRECTION[] = { -SFloat::_1, SFloat::_0, SFloat::_0 };
    const T VALUE_FOR_ORIGIN(VECTOR_COMPONENTS_ORIGIN);
    const T VALUE_FOR_DIRECTION(VECTOR_COMPONENTS_DIRECTION);
    Ray<T, T> RAY = Ray<T, T>(VALUE_FOR_ORIGIN, VALUE_FOR_DIRECTION);
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint1_ReturnedPointIsTheClosestOneToRayOrigin_Test, TTemplateTypes )
{
    ...
    float_z VECTOR_COMPONENTS_ORIGIN2[] = { SFloat::_4, SFloat::_2, SFloat::_2 };
    ...
    const Ray<T, T> RAY2 = Ray<T, T>(VALUE_FOR_ORIGIN2, VALUE_FOR_DIRECTION2.Normalize());
    ...
    float_z VECTOR_COMPONENTS_OUTPUT_POINT[] = { SFloat::_1, SFloat::_2, SFloat::_2 };
    const T EXPECTED_POINT = T(VECTOR_COMPONENTS_OUTPUT_POINT);
    float_z VECTOR_COMPONENTS_OUTPUT_POINT2[] = { SFloat::_3, SFloat::_2, SFloat::_2 };
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint2_ReturnsTwoIntersectionPointsWhenRayIntersectsTheOrbTwice_Test, TTemplateTypes )
{
    ...
    const EIntersections EXPECTED_RESULT = EIntersections::E_Two;
    float_z VECTOR_COMPONENTS_OUTPUT_POINT1[] = { SFloat::_1, SFloat::_2, SFloat::_2 };
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint2_ReturnsOnePointWhenOriginLiesOnTheSurfaceOfTheOrb_Test, TTemplateTypes )
{
    ...
    const EIntersections EXPECTED_RESULT = EIntersections::E_One;
    float_z VECTOR_COMPONENTS_OUTPUT_POINT1[] = { SFloat::_3, SFloat::_2, SFloat::_3 };
    const T EXPECTED_POINT1 = T(VECTOR_COMPONENTS_OUTPUT_POINT1);
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint2_ReturnsTwoPointsWhenOriginLiesOnTheSurfaceOfTheOrbAndTheRayPointsToInteriorOfOrb_Test, TTemplateTypes )
{
    ...
    const EIntersections EXPECTED_RESULT = EIntersections::E_Two;
    float_z VECTOR_COMPONENTS_OUTPUT_POINT1[] = { SFloat::_3, SFloat::_2, SFloat::_3 };
    float_z VECTOR_COMPONENTS_OUTPUT_POINT2[] = { SFloat::_1, SFloat::_2, SFloat::_3 };
    const T EXPECTED_POINT1 = T(VECTOR_COMPONENTS_OUTPUT_POINT1);
    const T EXPECTED_POINT2 = T(VECTOR_COMPONENTS_OUTPUT_POINT2);
    T OUTPUT_POINT1 = T::GetNullVector();
    T OUTPUT_POINT2 = T::GetNullVector();
...
ZTEST_CASE_TEMPLATE ( IntersectionPoint2_ReturnsOnePointWhenRayIsTangentToTheOrb_Test, TTemplateTypes )
{
    ...
    const EIntersections EXPECTED_RESULT = EIntersections::E_One;
    float_z VECTOR_COMPONENTS_OUTPUT_POINT1[] = { SFloat::_2, SFloat::_3, SFloat::_3 };
    const T EXPECTED_POINT1 = T(VECTOR_COMPONENTS_OUTPUT_POINT1);
    const T EXPECTED_POINT2 = T::GetNullVector();
    T OUTPUT_POINT1 = T::GetNullVector();
    T OUTPUT_POINT2 = T::GetNullVector();
