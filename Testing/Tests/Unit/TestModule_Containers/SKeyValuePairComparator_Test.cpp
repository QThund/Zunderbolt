...
ZTEST_SUITE_BEGIN( SKeyValuePairComparator_TestSuite )
...
ZTEST_CASE ( Compare_ReturnsPositiveOneWhenLeftOperandIsGreaterThanRightOperand_Test )
{
    ...
    const KeyValuePair<int, int> RIGHT_OPERAND(5, 0);
    const KeyValuePair<int, int> LEFT_OPERAND(10, 0);

    // [Execution]
    i8_z nComparisonResult = SKeyValuePairComparator<int, int>::Compare(LEFT_OPERAND, RIGHT_OPERAND);
...
ZTEST_CASE ( Compare_ReturnsNegativeOneWhenLeftOperandIsLowerThanRightOperand_Test )
{
    // [Preparation]
    const i8_z EXPECTED_VALUE_OF_COMPARISON = -1;
    const KeyValuePair<int, int> RIGHT_OPERAND(10, 0);
    const KeyValuePair<int, int> LEFT_OPERAND(5, 0);
...
ZTEST_CASE ( Compare_ReturnsZeroWhenLeftOperandIsEqualToRightOperand_Test )
{
    ...
    const KeyValuePair<int, int> RIGHT_OPERAND(10, 1);
    const KeyValuePair<int, int> LEFT_OPERAND(10, 2);
...
// End - Test Suite: SKeyValuePairComparator
ZTEST_SUITE_END()
