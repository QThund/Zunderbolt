...
#ifndef __EFILESYSTEMERROR__
#define __EFILESYSTEMERROR__
...
public:
    ...
    enum EnumType
    {
        E_Success = Z_ENUMERATION_MIN_VALUE, /*!< The operation completed successfully. */
...
public:
    ...
    EFileSystemError(const EFileSystemError::EnumType eValue) : m_value(eValue)
    {
    }
    ...
    EFileSystemError(const enum_int_z nValue) : m_value(scast_z(nValue, const EFileSystemError::EnumType))
    {
    }
    ...
    EFileSystemError(const char* szValueName)
    {
        *this = szValueName;
    }
    ...
    EFileSystemError(const EFileSystemError &eValue) : m_value(eValue.m_value)
    {
    }
    ...
    /// </returns>
    EFileSystemError& operator=(const enum_int_z nValue)
    {
        m_value = scast_z(nValue, const EFileSystemError::EnumType);
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    EFileSystemError& operator=(const char* szValueName)
    {
        bool bMatchFound = false;
        unsigned int uEnumStringIndex = 0;

        while(!bMatchFound && uEnumStringIndex < EFileSystemError::_GetNumberOfValues())
        {
            bMatchFound = strcmp(sm_arStrings[uEnumStringIndex], szValueName) == 0;
            ++uEnumStringIndex;
        }

        Z_ASSERT_ERROR(uEnumStringIndex < EFileSystemError::_GetNumberOfValues(), "The input string does not correspond to any valid enumeration value.");
    ...
    /// The enumerated type itself.
    /// </returns>
    EFileSystemError& operator=(const EFileSystemError::EnumType eValue)
    {
        m_value = eValue;
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    EFileSystemError& operator=(const EFileSystemError &eValue)
    {
    ...
    /// </returns>
    bool operator==(const EFileSystemError &eValue) const
    {
    ...
    {
        return m_value == scast_z(nValue, const EFileSystemError::EnumType);
    }
    ...
    /// </returns>
    bool operator==(const EFileSystemError::EnumType eValue) const
    {
    ...
    {
        static const ArrayBasic<const EnumType> ARRAY_OF_VALUES(sm_arValues, EFileSystemError::_GetNumberOfValues());
    ...
    /// </returns>
    operator EFileSystemError::EnumType() const
    {
        return m_value;
    }
...
private:
    ...
    /// </returns>
    inline static const char* _ConvertToString(const EFileSystemError::EnumType eValue)
    {
        Z_ASSERT_ERROR(scast_z(eValue, unsigned int) < EFileSystemError::_GetNumberOfValues(), "The enumeration value is not valid.");
...
private:
    ...
    /// </summary>
    static const EFileSystemError::EnumType sm_arValues[];

    /// <summary>
    /// The contained enumeration value.
    /// </summary>
    EFileSystemError::EnumType m_value;
