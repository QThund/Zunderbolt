...
#ifndef __EITERATIONDIRECTION__
#define __EITERATIONDIRECTION__
...
public:
    ...
    enum EnumType
    {
        E_Forward = Z_ENUMERATION_MIN_VALUE, /*!< Forward iteration, from the first element to the last one. */
...
public:
    ...
    EIterationDirection(const EIterationDirection::EnumType eValue) : m_value(eValue)
    {
    }
    ...
    EIterationDirection(const enum_int_z nValue) : m_value(scast_z(nValue, const EIterationDirection::EnumType))
    {
    }
    ...
    EIterationDirection(const char* szValueName)
    {
        *this = szValueName;
    }
    ...
    EIterationDirection(const EIterationDirection &eValue) : m_value(eValue.m_value)
    {
    }
    ...
    /// </returns>
    EIterationDirection& operator=(const enum_int_z nValue)
    {
        m_value = scast_z(nValue, const EIterationDirection::EnumType);
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    EIterationDirection& operator=(const char* szValueName)
    {
        bool bMatchFound = false;
        unsigned int uEnumStringIndex = 0;

        while(!bMatchFound && uEnumStringIndex < EIterationDirection::_GetNumberOfValues())
        {
            bMatchFound = strcmp(sm_arStrings[uEnumStringIndex], szValueName) == 0;
            ++uEnumStringIndex;
        }

        Z_ASSERT_ERROR(uEnumStringIndex < EIterationDirection::_GetNumberOfValues(), "The input string does not correspond to any valid enumeration value.");
    ...
    /// The enumerated type itself.
    /// </returns>
    EIterationDirection& operator=(const EIterationDirection::EnumType eValue)
    {
        m_value = eValue;
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    EIterationDirection& operator=(const EIterationDirection &eValue)
    {
    ...
    /// </returns>
    bool operator==(const EIterationDirection &eValue) const
    {
    ...
    {
        return m_value == scast_z(nValue, const EIterationDirection::EnumType);
    }
    ...
    /// </returns>
    bool operator==(const EIterationDirection::EnumType eValue) const
    {
    ...
    {
        static const ArrayBasic<const EnumType> ARRAY_OF_VALUES(sm_arValues, EIterationDirection::_GetNumberOfValues());
    ...
    /// </returns>
    operator EIterationDirection::EnumType() const
    {
        return m_value;
    }
...
private:
    ...
    /// </returns>
    inline static const char* _ConvertToString(const EIterationDirection::EnumType eValue)
    {
        Z_ASSERT_ERROR(scast_z(eValue, unsigned int) < EIterationDirection::_GetNumberOfValues(), "The enumeration value is not valid.");
...
private:
    ...
    /// </summary>
    static const EIterationDirection::EnumType sm_arValues[];

    /// <summary>
    /// The contained enumeration value.
    /// </summary>
    EIterationDirection::EnumType m_value;
